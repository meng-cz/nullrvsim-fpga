...
class NulCPUCtrlMP(cpunum: Int) extends Module {
    ...
    val init_timer_cnt = RegInit(0.U(30.W))
    when(state === STATE_INIT_WAIT) {
        when(init_timer_cnt < 250000000.U) {
            init_timer_cnt := init_timer_cnt + 1.U
        }.elsewhen(all_inited) {
            state := STATE_DO_INIT
    ...
    val perform_next_idx = RegInit(0.U(cpunum_bitwid.W))
    ...
    val satp_back = RegInit(VecInit(Seq.fill(cpunum)(0.U(64.W))))
    ...
    when(state === STATE_NEXT) {
        ...
        when(cnt(12)) { read_reg(0, satp_back(opidx)) }
        recover_regs(13, 3)
        when(cnt(16)) {
            cnt := 1.U 
            state := STATE_SEND_HEAD
    ...
    when(state === STATE_REDIR) {
        backup_regs(0, 2)
        when(cnt(2)) { write_reg(0, satp_back(opidx)) }
